...
void textline(int curposx, int curposy, char *stringdata, int fontindex,
              long textcolor) {
  // FIXME: implement printing text
  // for inspiration maybe see the frametime drawing routine
}
    ...
    if ((nY >= 0) && (nY < BHEIGHT)) {
      ...
      if ((nX >= 0) && (nX < BWIDTH)) {
        ...
        if (bpict[nY][nX] < nZ) {
          ...
          tcolor = ((tBlue << 16) + (tGreen << 8) + tRed) & 0x00FFFFFF;
  ...
  case ABSZ:
    ...
    for (int pr = (256 - 1); pr >= 0; pr--) {
      for (int pp = 0; pp < 256; pp++) {
        ...
        vert.pos[0] = tmp + pp;
        vert.pos[1] = HEIGHT - 304 + pr;
        vert.color = lpCols[PALSIZE - (pz & (PALSIZE - 1))];
        verts.push_back(vert);
      }
    }
    break;
...
void topLevelDisplay() {
  ...
  glColorPointer(3, GL_UNSIGNED_BYTE, sizeof(Vertex),
...
int main(int argc, char **argv) {
  ...
  // FIXME: don't. instead, implement keyboard controls
  programMode = 0;
  paintOnNextFrame = true;
